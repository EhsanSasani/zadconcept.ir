...
class SeoContractTests(TestCase):
    def setUp(self):
        ...
        # Migration 0013 intentionally seeds the canonical wedding category.
        # Tests must reuse that domain fixture instead of recreating a row that
        # violates Category's (section, slug) uniqueness contract.
        self.wedding_category, _ = Category.objects.update_or_create(
            slug="wedding",
            section=Category.Section.FLOWERS,
            defaults={"name": "عروسی", "is_active": True},
        )
    ...
    def test_query_policy_separates_filters_partials_and_pagination(self):
        filtered = self.client.get(reverse("bakery"), {"category": "pastry"})
    ...
    def test_sitemap_is_model_driven_canonical_and_direct(self):
        ...
        expected_paths = (
            ...
            self.wedding_category.get_absolute_url(),
